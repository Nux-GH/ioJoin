...
'''
https://stackoverflow.com/a/53342556
'''
...
<SelectableLabel>:
    ...
    canvas.before:
        Color:
            rgba: (.0, 0.3, .4, .5) if self.selected else (0, 0, 0, 1)
        Rectangle:
            pos: self.pos
            size: self.size
...
class RV(RecycleView):
    def __init__(self, **kwargs):
        ...
        for i1, i2 in paired_iter:
            ...
            self.data.append(
                d)  # can also be performed in a complicated one liner for those who like it tricky  # self.data = [{
            # 'label2': {'text': i1}, 'label3': {'text': i2}} for i1, i2 in zip(items_1, items_2)]

    def add(self, data, level='Info'):
        self.data.append({'label2': {'text': level}, 'label3': {'text': data}})
